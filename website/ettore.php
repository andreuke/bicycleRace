<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="icon" href="favicon.ico">

    <title></title>

    <!-- Bootstrap core CSS -->
    <link href="css/bootstrap.min.css" rel="stylesheet">
  </head>

  <body>
    <?php include("navbar.php") ?>
    <div class="container">
      <div class="row">
        <div class="page-header text-center">
          <h1>Ettore Trainiti</h1>
        </div>
      </div>
      <div class="col-md-4 col-md-offset-4">
          <a href="#" class="thumbnail">
            <img class="img-rounded" data-src="holder.js/250%x250" src="resources/ettore_250.jpg">
          </a>
      </div>
    </div>

    <div class="container">

      <div class="row">
        <div class="page-header">
          <h1>Week 1 <small> September 23rd - September 28th </small></h1>
        </div>
          <ul>
              <li>SQL Database Local Import attempt</li>
              <li>FTP Accounts set up</li>
              <li>UIC MySQL Database Request</li>
          </ul>
      </div>

      <div class="row">
        <div class="page-header">
          <h1>Week 2 <small>September 29th -October 5th </small></h1>
        </div>
          <ul>
              <li>Flexbox specifications readings</li>
              <li>MySQL Account set up</li>
              <li>MySQL Users set up</li>
              <li>Ordinal LineChart</li>
          </ul>
      </div>

      <div class="row">
        <div class="page-header">
          <h1>Week 3 <small>October 6th - October 12th </small></h1>
        </div>
        <ul>
          <li>LineChart refinements</li>
          <li>MultiSeries LineChart draft</li>
          <li>Numerical LineChart</li>
          <li>Date LineChart</li>
          <li>Year/Age LineChart</li>
          <li>Value aggregator for LineChart</li>
          <li>Hour Slider</li>
        </ul>
      </div>

      <div class="row">
        <div class="page-header">
          <h1>Week 4 <small>October 13th - October 19th </small></h1>
        </div>
        <ul>
            <li>Refactor</li>
            <li>Bug Fixes and Features</li>
        </ul>
      </div>

      <div class="row">
        <div class="page-header">
          <h1>Week 5 <small>October 20th - October 26th </small></h1>
        </div>
        <ul>
            <li>Weather LineChart</li>
            <li>Weather Box</li>
            <li>Findings</li>
            <li>Website</li>
            <li>Video</li>
        </ul>
      </div>

    </div>


    <?php include("footer.php") ?>


    <!-- Bootstrap core JavaScript
    ================================================== -->
    <!-- Placed at the end of the document so the pages load faster -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
    <script src="js/bootstrap.min.js"></script>
  </body>
</html>